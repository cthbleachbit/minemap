...
unsigned char byte_scale_reference[] = 
		{0x01, // byte type
...
int main() {
	...
	nbt -> payload = 0x01;
	unsigned char *bin_nbt = malloc(size_byte_nbt(nbt));
	generate_byte_nbt(bin_nbt, nbt);
	for (int i = 0; i < (signed int) sizeof(byte_scale_reference); i++) {
		printf("%x\t%x\n", bin_nbt[i], byte_scale_reference[i]);
	}
	assert(memcmp(bin_nbt, byte_scale_reference, sizeof(byte_scale_reference))==0);
	free(bin_nbt);
	return 0;
}
